...
#Aufgabe 01_qualitaetskontrolle; Musterloesung:
...
#Ueberpruefung: 
 ...
 muster_lsg  = try(qualitatetskontrolle_muster_lsg(precip = list(20L, 50L, 0.3, 0L, 0.7, 1L), stoertage = 1:2)) #Ergebnis der Musterloesung
...
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
 ...
 muster_lsg  = try(qualitatetskontrolle_muster_lsg(precip = 1:6, stoertage = 4:5)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test3
 einreichung = try(qualitatetskontrolle(precip = 1:6, stoertage = c(1L, 3L, 6L))) #Ergebnis der Einreichung
 muster_lsg  = try(qualitatetskontrolle_muster_lsg(precip = 1:6, stoertage = c(1L, 3L, 6L))) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
...
#Aufgabe 02_gebietsniederschlag; Musterloesung:
...
#Ueberpruefung: 
 ...
 muster_lsg  = try(gebietsniederschlag_muster_lsg(messungen = 1L)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test2
 einreichung = try(gebietsniederschlag(messungen = 5L)) #Ergebnis der Einreichung
 muster_lsg  = try(gebietsniederschlag_muster_lsg(messungen = 5L)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test3
 einreichung = try(gebietsniederschlag(messungen = 15L)) #Ergebnis der Einreichung
 muster_lsg  = try(gebietsniederschlag_muster_lsg(messungen = 15L)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
...
#Ueberpruefung: 
 ...
 muster_lsg  = try(egl1_muster_lsg(precip = 5L, egl = c(0.2, 0.5, 0.3), area = 3.6)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test2
 einreichung = try(egl1(precip = 10L, egl = c(0.1, 0.6, 0.3), area = 7.2)) #Ergebnis der Einreichung
 muster_lsg  = try(egl1_muster_lsg(precip = 10L, egl = c(0.1, 0.6, 0.3), area = 7.2)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test3
 einreichung = try(egl1(precip = 0L, egl = c(0.1, 0.6, 0.3), area = 10.8)) #Ergebnis der Einreichung
 muster_lsg  = try(egl1_muster_lsg(precip = 0L, egl = c(0.1, 0.6, 0.3), area = 10.8)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
...
#Ueberpruefung: 
 ...
 muster_lsg  = try(egl2_muster_lsg(precip = c(2L, 6L, 3L, 2L, 7L, 4L, 5L), egl = c(0.2, 0.5, 0.3))) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test2
 einreichung = try(egl2(precip = 2L, egl = c(0.1, 0.6, 0.2, 0.1))) #Ergebnis der Einreichung
 muster_lsg  = try(egl2_muster_lsg(precip = 2L, egl = c(0.1, 0.6, 0.2, 0.1))) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test3
 einreichung = try(egl2(precip = c(2L, 6L, 3L, 2L, 7L, 4L, 5L, 15L), egl = c(0.1, 0.6, 0.2, 0.1))) #Ergebnis der Einreichung
 muster_lsg  = try(egl2_muster_lsg(precip = c(2L, 6L, 3L, 2L, 7L, 4L, 5L, 15L), egl = c(0.1, 0.6, 0.2, 0.1))) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
...
#Ueberpruefung: 
 ...
 muster_lsg  = try(egl3_muster_lsg(precip = c(2L, 6L), egl = c(0.2, 0.5, 0.3), area = 3.6)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test2
 einreichung = try(egl3(precip = c(3L, 10L, 5L), egl = c(0.1, 0.6, 0.2, 0.1), area = 7.2)) #Ergebnis der Einreichung
 muster_lsg  = try(egl3_muster_lsg(precip = c(3L, 10L, 5L), egl = c(0.1, 0.6, 0.2, 0.1), area = 7.2)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe

 #test3
 einreichung = try(egl3(precip = c(6L, 3L, 4L), egl = c(0.2, 0.5, 0.3), area = 10.8)) #Ergebnis der Einreichung
 muster_lsg  = try(egl3_muster_lsg(precip = c(6L, 3L, 4L), egl = c(0.2, 0.5, 0.3), area = 10.8)) #Ergebnis der Musterloesung
if (identical2(einreichung, muster_lsg)) kontrollergebnis_t$richtig[1] = kontrollergebnis_t$richtig[1] + 1 #zaehle richtige Ergebnisse
if (class(einreichung) == "try-error") kontrollergebnis_t$nicht_aufrufbar[1] = kontrollergebnis_t$nicht_aufrufbar[1] + 1 #zaehle fehlerhafte Aufrufe
