...
# SET STREAM FLOW GAUGE ID
id = "02296750"

# Read meta data
meta = read.fwf(file="../data/mopex/usgs431.txt",
...
fname = paste("../data/mopex/Us_438_Daily/", id, ".dly", sep="")
...
# maxt: daily maximum temperature (monthly file: average minimum daily temperature) (in degC)
", sep="")

options(warn=1)
fname = paste(id, ".daily", sep="")
write(header, file=fname)
write.table(daily, file=fname, sep=";", append=TRUE, quote=FALSE, row.names=FALSE)
fname = paste(id, ".monthly", sep="")
write(header, file=fname)
